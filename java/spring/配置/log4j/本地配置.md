...
Configuration:  
  ...
  Appenders:  
    Console: # 节点用来定义输出到控制台的 Appender.      name: console  
      target: SYSTEM_OUT  # SYSTEM_OUT 或 SYSTEM_ERR, 一般只设置默认:SYSTEM_OUT.  
      PatternLayout:  
        Pattern: ${pattern}  # 输出的日志格式  
        disableAnsi: false  
        noConsoleNoAnsi: false  
    RollingFile: # 节点用来定义超过指定大小自动删除旧的创建新的的 Appender, 使用 DirectWriteRolloverStrategy 策略时,不需要配置 fileName      name: logFile  
